...
public class Houseboat {
	/*				I Think I Need a Houseboat
	  	...
	  	Total Submissions: 92244	Accepted: 40085
	  ...
	  data sets will be included (N). Each of the next N lines will contain the 
	  X and Y Cartesian coordinates of the land Fred is considering. These will 
	  be floating point numbers measured in miles. The Y coordinate will be 
	  non-negative. (0,0) will not be given.
	  
	  Output:
	  For each data set, a single line of output should appear. This line 
	  should take the form of: ¡°Property N: This property will begin eroding 
	  in year Z.¡± Where N is the data set (counting from 1), and Z is the 
	  first year (start from 1) this property will be within the semicircle 
	  AT THE END OF YEAR Z. Z must be an integer. After the last data set, 
	...
	public static void main(String[] args) {
		...
		while (true) {
			try {
				...
				if (count == 0) {
					...
					r = Math.sqrt(dd[0] * dd[0] + dd[1] * dd[1]);
					int year = 0;
					rc = Math.sqrt(2 * 50 * year / Math.PI);
					while (rc < r) {
						year++;
						rc = Math.sqrt(2 * 50 * year / Math.PI);
					}
		...
		for (int i = 0; i < result.length; i++) {
			System.out.println("Property " + (i + 1)
					+ ": This property will begin eroding in year " + result[i]
					+ ".");
		}
